...
import { useUpdateJobInterviewMutation } from "../../../redux/rtk/features/recruitment/jobInterview/jobInterviewApi";
import { useGetUsersQuery } from "../../../redux/rtk/features/user/userApi";
import BtnEditSvg from "../Button/btnEditSvg";

const JobInterviewEditPopup = ({ data }) => {
    const { Option } = Select;
    const [form] = Form.useForm();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const { id } = useParams("id");

    const [updateSingleJobInterview, { isLoading: updateLoading }] =
    ...
    const onFinish = async (values) => {
        ...
        const resp = await updateSingleJobInterview({ id, values });
    ...
    const initialValues = {
        jobApplicationId: data?.jobApplication.id,
        scheduleDate: dayjs(data?.scheduleDate),
        comment: data?.comment,
        assignedMembers: data?.jobInterviewMember?.map((item) => item.userId),
    };
                            ...
                            rules={[
                                {
                                    required: true,
                                    message:
                                        "please Input Job Interview Schedule Date",
                                },
                            ]}
                        >
                            ...
                            rules={[
                                {
                                    required: true,
                                    message:
                                        "please Input Job Interview Schedule Time",
                                },
                            ]}
                        >
                            ...
                            rules={[
                                {
                                    required: true,
                                    message:
                                        "please Input Job Interview comment",
                                },
                            ]}
                        >
...
export default JobInterviewEditPopup;
