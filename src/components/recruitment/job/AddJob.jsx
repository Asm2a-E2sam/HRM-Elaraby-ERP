...
import { useAddJobMutation } from "../../../redux/rtk/features/recruitment/job/jobApi";
...
const AddJob = () => {
  ...
  const { data: companyData, isLoading: companyLoading } = useGetSettingQuery();
  const { data: jobTypeData, isLoading: jobTypeLoading } = useGetJobTypesQuery({
    query: "all",
  });
  ...
  const [addSingleJob, { isLoading: addLoading }] = useAddJobMutation();
  ...
  const onFinish = async (values) => {
    try {
      ...
      const result = await addSingleJob(values);
      ...
      <UserPrivateComponent permission={"create-job"}>
        <div className="mr-top mt-5 p-5 ant-card " style={{ maxWidth: "100%" }}>
          ...
          >
            ...
            >
              <Col span={24} className="gutter-row form-color">
                <Form.Item
                  style={{
                    marginBottom: "12px",
                  }}
                ...
                >
                  ...
                  >
                    {companyData && (
                      <Option key={companyData?.id} value={companyData?.id}>
              ...
              <Col span={24} className="gutter-row form-color">
                ...
                >
                  <Input className="w-" placeholder="React.js Developer" />
                </Form.Item>
              </Col>
                ...
                >
                  ...
                  >
                    {jobWorkExperienceData &&
                      jobWorkExperienceData.map((item) => (
            ...
            {rangeValue === "exactSalary" ? (
              <>
                ...
                >
                  <Col span={12} className="gutter-row mt-6">
                    <Form.Item
                      style={{ marginBottom: "12px" }}
                      label="Salary"
                      name="exactSalary"
